...
_index_group_func_str = \
    ...
    {{
        CUPY_FOR(i, output.size()) {{
            ...
            const int w =                (sizeof({0}) * i / ostrides[6]) % oshape[6];

            int indexTUVW[5] = {{output_transform, input_transform, u, v, w}};
            int index[6] = {{output_channel, input_channel, T[indexTUVW], U[indexTUVW], V[indexTUVW], W[indexTUVW]}};
...
def index_group_func_kernel_3d(input, T, U, V, W, output):
    if input.dtype == 'float32':
        _index_group_func_kernel32.linear_launch(
            size=output.size,
            args=(input, T, U, V, W, output)
        )
...
def grad_index_group_func_kernel_3d(grad_output, T, U, V, W, grad_input):
    if grad_output.dtype == 'float32':
        _grad_index_group_func_kernel32.linear_launch(
            size=grad_output.size,
            args=(grad_output, T, U, V, W, grad_input)
        )
