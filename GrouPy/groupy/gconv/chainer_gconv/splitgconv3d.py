...
class SplitGConv3D(chainer.Link):
    """
    ...
    def __init__(self,
        ...
        :return:
        """
        super(SplitGConv3D, self).__init__()
        ...
        self.ksize = ksize
        self.stride = stride if hasattr(stride, '__getitem__') else (stride, stride, stride)
        self.pad = pad if hasattr(pad, '__getitem__') else (pad, pad, pad)
        self.use_cudnn = use_cudnn
        self.flat_channels = flat_channels

        w_shape = (self.out_channels, self.in_channels, self.input_stabilizer_size, self.ksize, self.ksize, self.ksize)
        ...
        if initialW is not None:
            ...
            self.W.data[:] = self.xp.random.normal(
                0, wscale * math.sqrt(1. / (self.input_stabilizer_size * self.ksize ** 3 * self.in_channels)),
                w_shape
            ).astype(self.dtype)
    ...
    def __call__(self, x):
        ...
        tw_shape = (self.out_channels * self.output_stabilizer_size,
                    ...
                    self.ksize, self.ksize, self.ksize)
        ...
        if not self.flat_channels:
            ...
            in_nz, in_ny, in_nx = x.data.shape[-3:]
            x = F.reshape(x, (batch_size, self.in_channels * self.input_stabilizer_size, in_nz, in_ny, in_nx))

        # Perform the 3D convolution
        y = F.convolution_3d(x, tw, b=None, stride=self.stride, pad=self.pad, use_cudnn=self.use_cudnn)
        ...
        batch_size, _, nz_out, ny_out, nx_out = y.data.shape
        y = F.reshape(y, (batch_size, self.out_channels, self.output_stabilizer_size, nz_out, ny_out, nx_out))

        # Add a bias to each G-feature map
        if self.usebias:
            bb = F.Reshape((1, self.out_channels, 1, 1, 1, 1))(self.b)
            y, b = F.broadcast(y, bb)
            y = y + b

        # Flatten feature channels if needed
        if self.flat_channels:
            n, nc, ng, nx, ny, nz = y.data.shape
            y = F.reshape(y, (n, nc * ng, nx, ny, nz))
