...
def check_c4_z2_conv_equivariance():
    ksize = 3
    im = np.random.randn(2, ksize, ksize, 1)
...
def check_c4_c4_conv_equivariance():
    ksize = 3
    im = np.random.randn(2, ksize, ksize, 4)
    x, y = make_graph('C4', 'C4', ksize)
...
def make_graph(h_input, h_output, ksize):
    ...
    x = tf.placeholder(tf.float32, [None, ksize, ksize, 1 * nti])
